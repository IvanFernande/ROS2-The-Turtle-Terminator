...
class SpawnerNode(Node):
    def __init__(self):
        ...
        self.spawner_timer = self.create_timer(
            1.0, self.spawn_function)
        self.turtle_taken_service_ = self.create_service(TurtleTaken, "turtle_taken", self.callback_turtle_taken)

        self.get_logger().info("Turtles are now spawning random. CATCH THEM ALL!")
    
    def callback_turtle_taken(self, request, response):
